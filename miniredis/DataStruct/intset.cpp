...
IntSet* IntSet::insert(int64_t value)
{
	...
	if(index != new_is->length-1)
		memmove(new_is->content + (index + 1) * new_is->encoding, new_is->content + index * new_is->encoding, (length - index) * new_is->encoding);
...
IntSet* IntSet::remove(int64_t value)
{
	...
	memmove(content + index * encoding, content + (index + 1) * encoding, (length - index - 1) * encoding);
	IntSet* new_is = resize(this, length - 1);
	return new_is;
}
