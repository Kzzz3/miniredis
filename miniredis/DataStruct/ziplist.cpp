...
{
    ZipList* zl = Allocator::create<ZipList>();
    zl->total_bytes = 0;
    zl->last_offset = 0;
    zl->items_num = 0;
...
ZipList* ZipList::pop_back()
{
    ...
    ZipList* new_zl = Allocator::recreate_with_extra<ZipList>(this, total_bytes, last_offset);
...
ZipList* ZipList::pop_front()
{
    ...
    ZipList* new_zl =
        Allocator::recreate_with_extra<ZipList>(this, total_bytes, total_bytes - entry_size);
...
ZipList* ZipList::insert(uint8_t* p, uint8_t* str, size_t len)
{
    ...
    ZipList* new_zl =
        Allocator::recreate_with_extra<ZipList>(this, total_bytes, total_bytes + new_entry_size);

    memmove(new_zl->buf + offset + new_entry_size, new_zl->buf + offset,
            new_zl->total_bytes - offset);
    entryEncode(new_zl->buf + offset, new_entry);

    new_zl->total_bytes += new_entry_size;
    new_zl->last_offset = new_zl->items_num == 0 ? 0
                          : offset == new_zl->total_bytes - new_entry_size
                              ? offset
                              : last_offset + new_entry_size;
    new_zl->items_num++;
    new_zl->buf[new_zl->total_bytes] = ZIPLIST_END;

    return adjustSubsequentNodes(new_zl, offset + new_entry_size,
                                 new_entry_size - new_entry.prevrawlen);
}
...
bool tryIntEncode(uint8_t* str, size_t len, ZlEntry& entry)
{
    ...
    try
    {
        ...
        if (!value.has_value())
            return false; // Ensure full match
...
ZipList* adjustSubsequentNodes(ZipList* zl, size_t offset, int diff)
{
    ...
    while (ret->buf[offset] != ZIPLIST_END)
    {
        ...
        if (diff < 0)
        {
            memmove(ret->buf + offset + new_entry_size, ret->buf + offset + old_entry_size,
                    ret->total_bytes - offset - old_entry_size);
            new_zl = Allocator::recreate_with_extra<ZipList>(ret, ret->total_bytes,
                                                             ret->total_bytes + diff);
        }
        else
        {
            new_zl = Allocator::recreate_with_extra<ZipList>(ret, ret->total_bytes,
                                                             ret->total_bytes + diff);
            memmove(new_zl->buf + offset + new_entry_size, new_zl->buf + offset + old_entry_size,
                    ret->total_bytes - offset - old_entry_size);
        }
